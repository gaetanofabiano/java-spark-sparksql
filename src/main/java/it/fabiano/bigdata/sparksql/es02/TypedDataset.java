package it.fabiano.bigdata.sparksql.es02;
...
import it.fabiano.bigdata.sparksql.es01.ResponsePOJO;
...
public class TypedDataset {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Dataset<ResponsePOJO> typedDataset = responseWithSelectedColumns.as(Encoders.bean(ResponsePOJO.class));
        ...
        typedDataset.filter((FilterFunction<ResponsePOJO>) response -> response.getCountry().equals("Afghanistan")).show();
        ...
        typedDataset.filter((FilterFunction<ResponsePOJO>)response -> response.getAgeMidPoint() !=null &&
        ...
        typedDataset.filter((FilterFunction<ResponsePOJO>) response -> response.getSalaryMidPoint() != null)
                    .groupBy("country")
                    .agg(avg(SALARY_MIDPOINT), max(AGE_MIDPOINT))
                    .show();

        System.out.println("=== Group by salary bucket ===");
        typedDataset.map((MapFunction<ResponsePOJO, Integer>) response -> response.getSalaryMidPoint() == null ?
                                                                      null :
